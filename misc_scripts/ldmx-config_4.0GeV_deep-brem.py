...
b_thresh = 2500.0 #float(sys.argv[1])*500.0 + 1000.0 #optimized for batch job running over multiple bias thresholds
p_sorter =  1500.0 #float(sys.argv[2]) #4990.0 
...
sim.actions.extend([
   ...
   filters.TaggerVetoFilter(thresh=3800.),
   filters.PrimaryToEcalFilter(50.0),
   filters.DeepEcalProcessFilter(bias_threshold=b_thresh, processes=["conv","phot)"], ecal_min_Z=400.0, require_photon_fromTarget=True)
   ])
